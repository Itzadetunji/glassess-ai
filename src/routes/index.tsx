...
import {
	FrameGalleryModal,
	type FrameGalleryState,
} from "#/components/frame-gallery-modal.tsx";
import { FrameTypeSelectTile } from "#/components/frame-image.tsx";
import { TryOnPreviewImage } from "#/components/try-on-preview-image.tsx";
...
import { downloadDataUrl } from "#/lib/download-data-url.ts";
...
export function Home() {
	...
	const [frameGallery, setFrameGallery] = useState<FrameGalleryState>(null);
	...
	const frameTypeKeys = Object.keys(FRAME_TYPES) as FrameTypeKey[];
		...
		<TooltipProvider delayDuration={200}>
			<div className="flex-1 flex flex-col items-center justify-center">
									...
									{(field) => (
										<>
											<div className="grid gap-2 overflow-hidden">
												...
												<div className="flex items-center gap-4 overflow-x-scroll p-1 no-scrollbar">
													{frameTypeKeys.map((key) => (
														<FrameTypeSelectTile
															key={key}
															frameType={key}
								...
								{(isSubmitting) => (
									<div className="flex w-full gap-2">
										...
										<Button
											type="button"
											variant="outline"
											className="flex-1 gap-2 sm:min-w-36"
											disabled={tryOn.data?.status !== "success"}
											onClick={() => {
												if (tryOn.data?.status !== "success") return;
												downloadDataUrl(
													tryOn.data.data.image,
													"glassess-try-on",
												);
											}}
										>
				...
				<FrameGalleryModal
					...
					onGalleryChange={setFrameGallery}
				/>
			</div>
		</TooltipProvider>
	);
